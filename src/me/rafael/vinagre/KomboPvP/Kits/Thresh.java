...
public class Thresh implements Listener, CommandExecutor {
	...
	public void clicar(PlayerInteractEvent e) {
		...
		if (((e.getAction() == Action.RIGHT_CLICK_AIR) || (e.getAction() == Action.RIGHT_CLICK_BLOCK))
				&& (p.getItemInHand().getType() == Material.TRIPWIRE_HOOK) && (Array.kit.get(p) == "Thresh"))
			if ((!Tempo.containsKey(p.getName()))
					|| (((Long) Tempo.get(p.getName())).longValue() <= System.currentTimeMillis())) {
				e.setCancelled(true);
				p.updateInventory();
				Snowball tiro = (Snowball) p.launchProjectile(Snowball.class);
				Vector vec = p.getLocation().getDirection();
				tiro.setVelocity(
						new Vector(vec.getX() * 1.0D * 3.5D, vec.getY() * 1.0D * 4.0D, vec.getZ() * 1.0D * 3.5D));
				...
				p.sendMessage(ChatColor.RED + "Faltam "
						+ TimeUnit.MILLISECONDS
								.toSeconds(((Long) Tempo.get(p.getName())).longValue() - System.currentTimeMillis())
	...
	public void aplicar(EntityDamageByEntityEvent e) {
		if (((e.getEntity() instanceof Player)) && ((e.getDamager() instanceof Snowball))) {
			...
			if ((tiro.getShooter() instanceof Player)) {
				...
				if ((tiros.containsKey(shooter.getName())) && (tiro == tiros.get(shooter.getName()))) {
