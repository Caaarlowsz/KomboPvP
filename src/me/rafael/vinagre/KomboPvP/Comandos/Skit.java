...
public class Skit implements Listener, CommandExecutor {
	...
	public boolean onCommand(CommandSender sender, Command cmd, String CommandLabel, String[] args) {
		...
		if ((cmd.getName().equalsIgnoreCase("skit")) && ((p.hasPermission("kitpvp.skit")))) {
			if (args.length == 0) {
				p.sendMessage((Main.getPlugin().getConfig().getString("NomeServer").replace("&", "§"))
						+ " §7» §cUse /skit criar|aplicar [nome]|[raio]");
				return true;
			}
			if (args[0].equalsIgnoreCase("criar")) {
				if (args.length == 1) {
					p.sendMessage((Main.getPlugin().getConfig().getString("NomeServer").replace("&", "§"))
							+ " §7» §cUse /skit criar [nome]");
					return true;
				}
				String name = args[1];
				kit.put(name, p.getInventory().getContents());
				armadura.put(name, p.getInventory().getArmorContents());
				p.sendMessage((Main.getPlugin().getConfig().getString("NomeServer").replace("&", "§")) + " §7» §aKit "
						+ args[1] + " §acriado com sucesso!");
				return true;
			}
			if (args[0].equalsIgnoreCase("aplicar")) {
				if (args.length <= 2) {
					p.sendMessage((Main.getPlugin().getConfig().getString("NomeServer").replace("&", "§"))
							+ " §7» §cUse /skit aplicar [kits] [distancia]");
					return true;
				}
				String name = args[1];
				if ((!this.kit.containsKey(name)) && (!this.armadura.containsKey(name))) {
					p.sendMessage((Main.getPlugin().getConfig().getString("NomeServer").replace("&", "§"))
							+ " §7» §cKit " + name + " nao encontrado!");
					return true;
				}
				if (isInt(args[2])) {
					...
					p.sendMessage((Main.getPlugin().getConfig().getString("NomeServer").replace("&", "§"))
							+ " §7» §aKit " + name + " aplicado para jogadores em um raio de " + numero + " blocos");
					return true;
				}
			}
		}
