...
public class AutoSoup implements CommandExecutor, Listener {
	...
	public boolean onCommand(CommandSender sender, Command cmd, String Label, String[] args) {
		...
		if (cmd.getName().equalsIgnoreCase("autosoup")) {
			...
			Bukkit.getServer().getScheduler().scheduleSyncDelayedTask(me.rafael.vinagre.KomboPvP.Main.getPlugin(),
					new Runnable() {
						public void run() {
					...
					new Runnable() {
						public void run() {
							...
							testando.getInventory()
									.setArmorContents((ItemStack[]) AutoSoup.armadura.get(testando.getName()));
