...
public class Sortear implements Listener, CommandExecutor {
	boolean caixa = false;
	...
	public boolean onCommand(CommandSender sender, Command cmd, String CommandLabel, String[] args) {
		...
		if (cmd.getName().equalsIgnoreCase("sortear")) {
			...
			if (args[0].equalsIgnoreCase("caixa")) {
				...
				Bukkit.broadcastMessage(
						"§e§lDesert§f§lPvP §7» §aSorteando uma Caixa Normal daqui 1 Minuto ! \nPara participar fique online !");
				Bukkit.broadcastMessage(" ");
				caixa = true;
					...
					public void run() {
						if (caixa == true) {
							...
							if (emsabugado.size() < 5) {
								Bukkit.broadcastMessage(" ");
								Bukkit.broadcastMessage(
										"§e§lDesert§f§lPvP §7» §cSorteio cancelado pela falta de jogadores !");
								Bukkit.broadcastMessage(" ");
								return;
							}
							...
							Bukkit.broadcastMessage(" ");
							Bukkit.broadcastMessage(
									"§e§lDesert§f§lPvP §7» §aSorteio finalizado ! \nGanhador: §f" + p2.getName()
											+ " §a!");
							Bukkit.broadcastMessage(" ");
//			        	  try {
