...
public class Eventos implements Listener {
	...
	public void onDropSword(PlayerDropItemEvent e) {
		if ((e.getItemDrop().getItemStack().getType() == Material.WOOD_SWORD) || ((e.getItemDrop().getItemStack()
				.getType() == Material.SAND)
				|| ((e.getItemDrop().getItemStack().getType() == Material.CLAY_BRICK) || ((e.getItemDrop()
						.getItemStack().getType() == Material.PACKED_ICE)
						|| ((e.getItemDrop().getItemStack().getType() == Material.SAND) || ((e.getItemDrop()
								.getItemStack().getType() == Material.IRON_FENCE)
								|| ((e.getItemDrop().getItemStack().getType() == Material.IRON_INGOT) || ((e
										.getItemDrop().getItemStack().getType() == Material.BOOK)
										|| ((e.getItemDrop().getItemStack().getType() == Material.SLIME_BALL) || ((e
												.getItemDrop().getItemStack().getType() == Material.IRON_FENCE)
												...
												|| (e.getItemDrop().getItemStack().getType() == Material.NAME_TAG)
												|| (e.getItemDrop().getItemStack().getType() == Material.SADDLE)
												|| (e.getItemDrop().getItemStack().getType() == Material.LEATHER)
												...
												|| (e.getItemDrop().getItemStack()
														.getType() == Material.FLINT_AND_STEEL)
												|| (e.getItemDrop().getItemStack().getType() == Material.NETHER_FENCE)
												|| (e.getItemDrop().getItemStack().getType() == Material.PUMPKIN_SEEDS)
												...
												|| (e.getItemDrop().getItemStack().getType() == Material.TRIPWIRE_HOOK)
	...
	public void reparar(PlayerInteractEvent e) {
		Player p = e.getPlayer();
		if (((Array.used.contains(p.getName())) && (e.getAction() == Action.RIGHT_CLICK_AIR)
	...
	public void gerar(SignChangeEvent e) {
		...
		if (e.getLine(0).equalsIgnoreCase("[Sopas]")) {
			e.getBlock().breakNaturally();
			Villager bat = (Villager) e.getBlock().getWorld().spawnEntity(e.getBlock().getLocation(),
					EntityType.VILLAGER);
			e.getBlock().breakNaturally();
	...
	public void comando(PlayerCommandPreprocessEvent e) {
		Player p = e.getPlayer();
		if (!p.hasPermission("kitpvp.admin")) {
			if ((Array.warp.contains(p.getName())) && (!e.getMessage().equals("/spawn"))
					&& (!e.getMessage().equals("/tell"))) {
				e.setCancelled(true);
				p.sendMessage("§7» §cComando nao permitido nas warps !");
			}
			if (e.getMessage().equals("/pl") || e.getMessage().equals("/ver") || e.getMessage().equals("/plugins")
					|| e.getMessage().equals("/help")) {
				e.setCancelled(true);
			}
		...
		if (e.getResult() == PlayerLoginEvent.Result.KICK_FULL) {
			if (p.hasPermission("kitpvp.vip")) {
				...
				e.setKickMessage("§c§lOps \n§cServidor cheio, compre VIP e garanta seu SLOT !\n§f"
						+ ((Main.getPlugin().getConfig().getString("Site").replace("&", "§"))));
			}
